...
- **📖 Stories** — short graded readers. Every word is tappable (with smart stem matching for inflected forms). Finish a story for +15 XP (once per story per day). The **🌐 Random Wiki** chip pulls a random Russian Wikipedia intro (free API, needs internet) as untranslated "wild" reading — +5 XP per article. In wiki mode, tapped words are translated live via the free Google Translate endpoint (any word form, no API key), falling back to the bundled frequency-deck dictionaries when offline.
